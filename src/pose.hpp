...
namespace poseEstimation{
    ...
    public:
        ...
        std::vector<std::vector<ZAJX::CaveAlgo::OneEuroFilter>> vec_vec_euro_filters;
        float confidence;
        cv::Rect bbox;
        int track_id;
        ...
        Pose(std::vector<cv::Point> keypoints, float confidence=1):
            keypoints(keypoints),confidence(confidence){
                ...
                this->track_id = -1;
                this->vec_vec_euro_filters.resize(num_kpts, std::vector<ZAJX::CaveAlgo::OneEuroFilter>(2));
                for (int f_id = 0; f_id < this->vec_vec_euro_filters.size(); ++f_id)
                {
                    ZAJX::CaveAlgo::OneEuroFilter oneEuro_filter_x;
                    ZAJX::CaveAlgo::OneEuroFilter oneEuro_filter_y;
                    this->vec_vec_euro_filters[f_id][0] = oneEuro_filter_x;
                    this->vec_vec_euro_filters[f_id][1] = oneEuro_filter_y;
                }
        }

        Pose& operator=(const Pose& obj){
            this->keypoints = obj.keypoints;
            this->vec_vec_euro_filters = obj.vec_vec_euro_filters;
        ...
        void draw(cv::Mat &img, bool draw_track_id = false){
            for(int part_id = 0 ;part_id < this->BODY_PARTS_KPT_IDS.size() - 2 ; part_id++){
                ...
                if(global_kpt_a_id != -1){
                    ...
                    cv::circle(img ,p_a ,3, bone_color, -1);
                }
                ...
                if(global_kpt_b_id != -1){
                    ...
                    cv::circle(img ,p_b ,3, bone_color, -1);
                }
                if(global_kpt_a_id != -1 && global_kpt_b_id != -1){
                    cv::Point p_a = this->keypoints[kpt_a_id];
                    cv::Point p_b = this->keypoints[kpt_b_id];
                    cv::line(img ,p_a ,p_b, bone_color, 4);
                }
            }
    ...
    private:
        cv::Scalar bone_color = { 255, 255, 0 };
    ...
    class poseTracker{        
    public:
        void track(std::vector<Pose> &current_poses, int threshold = 3, bool smooth = true){
            ...
            for(int i = 0; i < current_poses.size(); i++){
               ...
               if (smooth)
               {
                   for (int kp_id = 0; kp_id < current_poses[i].keypoints.size(); ++kp_id)
                   {
                       if (std::fabs(current_poses[i].keypoints[kp_id].x) != -1)
                           continue;

                       if (best_matched_pose_id  !=  -1 && std::fabs(this->previous_poses[best_matched_id].keypoints[kp_id].x) != -1)
                       {
                           current_poses[i].vec_vec_euro_filters[kp_id] = this->previous_poses[best_matched_id].vec_vec_euro_filters[kp_id];
                       }
                       current_poses[i].keypoints[kp_id].x = current_poses[i].vec_vec_euro_filters[kp_id][0].filter(current_poses[i].keypoints[kp_id].x);
                       current_poses[i].keypoints[kp_id].y = current_poses[i].vec_vec_euro_filters[kp_id][1].filter(current_poses[i].keypoints[kp_id].y);
                   }
               }
               current_poses[i].bbox = current_poses[i].get_bbox();;
            }
